...
// Tests CRUD operations: create document (via API), list, get single,
// delete (soft-archive), and the archive/restore flow.
...
describe('Document Listing (GET /api/documents)', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockRequireAuth.mockResolvedValue({ clerkUserId: 'user_abc', tenantId: 'tenant_xyz' });
  });

  it('returns documents for the authenticated tenant', async () => {
    mockDb.select.mockReturnValue(makeThenableSelect(sampleDocs));
    ...
    expect(json[0].id).toBe('doc_001');
    expect(json[1].title).toBe('Offer B');
  });
  ...
  it('does NOT return documents from other tenants', async () => {
    mockDb.select.mockReturnValue(makeThenableSelect([])); // Other tenant's docs filtered by DB query
...
describe('Document Dashboard (GET /api/dashboard)', () => {
  ...
  it('returns dashboard data with recent documents and usage info', async () => {
    // Mock the dashboard route's DB calls: recent docs + subscription + tenant
    // The route makes multiple queries — first for docs, then subscription, then tenant
