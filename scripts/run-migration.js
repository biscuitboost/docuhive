/**
 * Apply the 0005 notifications migration to the production DB.
...
async function main() {
  ...
  // Parse the URL manually and reconstruct
  const uStart = rawUrl.indexOf("://");
  const protocol = rawUrl.substring(0, uStart + 3); // postgresql://
  ...
  const sqlPath = path.join(__dirname, "..", "lib", "db", "migrations", "0005_fearless_ma_gnuci.sql");
